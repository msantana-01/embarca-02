...
void controle_leds(char tecla) {
    ...
    //casos para cada opção de tecla
    switch (tecla) {
        case TECLA_A:
...
void controle_buzzer(char tecla) {
    if (tecla == TECLA_HASH) {
        ...
        sleep_ms(1000);             // Buzzer ligado por 1 segundo e depois desligar
...
int main() {
    ...
    while (true) {
        ...
        if (tecla) {  // Se uma tecla foi pressionada
            ...
            controle_leds(0);  // Desliga os LEDs
            controle_buzzer(0);  // Desliga o buzzer
        }
    }
